...
let btn_annuler = document.querySelector('.btn-annuler-stage')
...
btn_ajout_user.addEventListener('click', function () {
    dark.style.display = "block"
    document.querySelector('.btn-annuler-user').addEventListener('click', function () { dark.style.display = "none" })
})
...
document.querySelectorAll('.btn-modifier-user').forEach(btn => {
    btn.addEventListener('click', function () {
        ...
        const prenom = row.children[2].textContent.trim();
        const email = row.children[3].textContent.trim();
        const role = row.children[4].textContent.trim();

        dark.style.display = "block";

        const form = dark.querySelector('form');
        form.setAttribute('data-id', id);

        form.lastname.value = nom;
        form.firstname.value = prenom;
        form.email.value = email;
        form.role.valueOf = role;

        dark.querySelector('form').lastElementChild.textContent = "Modifier";
        dark.querySelector('form').lastElementChild.previousElementSibling.addEventListener('click', function(){dark.style.display = "none"})
    });
});

// Supprimer un utilisateur 

document.querySelectorAll('.btn-supprimer-user').forEach(btn => {
    btn.addEventListener('click', function () {
        const id = this.getAttribute('data-id');
        if (confirm("Voulez-vous vraiment supprimer cet utilisateur ?")) {
            fetch('supprimer_user.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                },
